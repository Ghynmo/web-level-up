 ...
 */
 ...
 cli.responders.help = function(){
   ...
   var commands = {
     ...
     'Create CRUD --{objectName}' : 'Creating new handler object file',
   };
 ...
 // Post User
 cli.responders.inputUser = function(){

  console.log('input the user [fname,lname,phone,password');
  ...
  _interface.on('line', function(str){
    ...
    var array = str.split(',');
    var result = handlers._users.postUsercli(array)
    console.log(result)
...
// Create new handler object file that can do CRUD
cli.responders.createCrud = function(str){

template_file = path.join(__dirname,'/../lib/handlers/crudTemplates.js');
handlers_dir = path.join(__dirname,'/../lib/handlers');
router_dir = path.join(__dirname,'/../lib/routers.js');
...
var arr = str.split('--');
  ...
  if(fileName){
    // create folder if folder hasn't created
    // if(!fs.existsSync(handlers_dir)){
    //   fs.mkdirSync(handlers_dir);
    // }
    console.log('get file name')
    fs.readFile(template_file,'utf8',function(err, data){
      if(err){
        console.log(err+'error read template file')
      } else {
        var newdata = data.toString().replace(/crud_template_name/gi, fileName)
        fs.writeFile(handlers_dir+'/'+fileName+'.js',newdata,function(){
          if(err){
            console.log(err+'error write')
          } else {
            console.log('success creating new file')

            fs.readFile(router_dir,'utf8',function(err, data){
              if(err){
                console.log(err+'error read template file')
              } else {
                var obj = {
                  'Automatic Dependencies will inserted below ' : `Automatic Dependencies will inserted below 
  var ${fileName} = require("./handlers/${fileName}")`,
                  'Automatic Router will inserted below' : `Automatic Router will inserted below
  "${fileName}": ${fileName}.${fileName},`,
                }
  
                var RE = new RegExp(Object.keys(obj).join("|"), "gi");
    
                var newRouter = data.toString().replace(RE, function (matched) {
                  return obj[matched];
                });
    
                fs.writeFile(router_dir, newRouter, function (err) {
                  if(err){
                    console.log(err+'error modifying new route')
                  } else {
                    console.log('success modifying new route')
                  }
                });
